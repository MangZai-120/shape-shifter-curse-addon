package net.jackcooper.shapeShifterCurseAddon.command;
...
import net.jackcooper.shapeShifterCurseAddon.palette.PaletteCodec;
...
public class SscAddonCommands {
						...
						.then(CommandManager.argument("book_id", StringArgumentType.string())
								.suggests((context, builder) -> {
									...
									return CommandSource.suggestMatching(
											net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getBookIds(),
											builder
									);
								})
	...
	private static int mancianimaAssaultReset(CommandContext<ServerCommandSource> ctx, ServerPlayerEntity player) {
		...
		net.jackcooper.shapeShifterCurseAddon.ability.MancianimaAssaultState state =
				net.jackcooper.shapeShifterCurseAddon.ability.MancianimaAssaultState.get(srv);
	...
	private static int setMana(CommandContext<ServerCommandSource> context, Collection<ServerPlayerEntity> targets, int amount) {
		...
		for (ServerPlayerEntity player : targets) {
			...
			if (soulMax > 0) {
				...
				net.jackcooper.shapeShifterCurseAddon.ability.AnubisWolfSpSoulEnergy.setEnergy(player, clamped);
				updated = true;
			}
	...
	private static int debugFormInfo(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		if (component == null) {
			...
			if (currentForm == null) {
				...
				debugInfo.append("Phase: ").append(currentForm.getPhase().name()).append("\n");
				player.sendMessage(Text.translatable("command.ssc_addon.debug_form.phase", String.valueOf(currentForm.getPhase().name())).formatted(Formatting.AQUA), false);
	...
	private static int debugAnim(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		net.jackcooper.shapeShifterCurseAddon.network.SscAddonNetworking.sendAnimDebugToggle(player);
		return 1;
	}
	...
	private static int giveStoryBookByIdInternal(CommandContext<ServerCommandSource> context, String language) throws CommandSyntaxException {
		...
		net.minecraft.item.ItemStack book = net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getStoryBookById(bookId, language);
		...
		// 获取书籍信息用于显示
		net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.BookData bookData =
				net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getBookDataById(bookId, language);
	...
	private static int listBooksInternal(CommandContext<ServerCommandSource> context, String language) throws CommandSyntaxException {
		...
		if (language != null && !language.isEmpty()) {
			bookIds = net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getBookIds(language);
		} else {
			bookIds = net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getBookIds();
		}
	...
	private static int reloadBooks(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		try {
			net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.reloadBooks();
			int bookCount = net.jackcooper.shapeShifterCurseAddon.loot.StoryBookLoot.getBookCount();
